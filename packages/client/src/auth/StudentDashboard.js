...
import { renderStudentCourses } from '../lessons/student-courses-page.js';
...
export function renderStudentDashboard(container, apiClient, { initialTab = 'exercises' } = {}) {
    ...
    <div class="student-layout">
      <aside class="sidebar student-sidebar">
        ...
        <nav class="sidebar-nav">
          ...
          <button class="sidebar-nav-item student-nav-item ${activeTab === 'courses' ? 'active' : ''}" data-tab="courses" title="Courses">
            <svg class="sidebar-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>
      ...
      <nav class="mobile-tab-bar">
        <div class="mobile-tab-pill">
          <button class="mobile-tab ${activeTab === 'exercises' ? 'mobile-tab-active' : ''}" data-tab="exercises">
          ...
          <button class="mobile-tab ${activeTab === 'performance' ? 'mobile-tab-active' : ''}" data-tab="performance">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>
            <span>STATS</span>
          </button>
          <button class="mobile-tab" id="mobile-logout-btn">
  ...
  const mobileLogout = container.querySelector('#mobile-logout-btn');
  if (mobileLogout) {
    mobileLogout.addEventListener('click', () => authManager.logout());
  }

  // Both sidebar nav and mobile tab bar navigation
  container.querySelectorAll('[data-tab]').forEach(btn => {
    btn.addEventListener('click', () => {
      const tab = btn.dataset.tab;
      if (tab === 'exercises' || tab === 'performance' || tab === 'courses') {
        const newHash = tab === 'performance' ? '/performance' : tab === 'courses' ? '/courses' : '/my-exercises';
  ...
  async function renderTab() {
    ...
    try {
      if (activeTab === 'exercises') {
        ...
        await renderStudentCourses(contentEl, apiClient);
