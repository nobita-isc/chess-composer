...
function validateVideoUrl(v) {
  if (!v) return null
  try { const u = new URL(v); return ['http:', 'https:'].includes(u.protocol) ? null : 'URL must be http or https' }
...
export function createContentItem({ item: initialItem, onPatch, onDelete }) {
  ...
  el.className = 'ci-root'
  el.style.cssText = 'border:1px solid #c7d2fe;border-radius:10px;background:#fff;overflow:hidden;margin-bottom:8px'
    ...
    return `
      <div class="ci-header" style="display:flex;align-items:center;gap:10px;padding:12px 14px;cursor:pointer;user-select:none">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:11px;color:#94a3b8;margin-top:1px">${escapeHtml(item.video_url) || 'No URL'}</div>
        </div>
        <span style="font-size:10px;font-weight:700;padding:2px 7px;border-radius:99px;background:#eef2ff;color:#4f46e5;flex-shrink:0">VIDEO</span>
      ...
      <div class="ci-body" style="padding:0 14px 14px;display:flex;flex-direction:column;gap:10px;border-top:1px solid #e2e8f0">
        ...
        <div>
          ...
          <input class="ci-url" type="url" value="${escapeHtml(item.video_url)}" placeholder="https://youtube.com/watch?v=..."
            style="width:100%;padding:8px 10px;border:1px solid ${urlError ? '#fca5a5' : '#d1d5db'};border-radius:6px;font-size:13px;font-family:inherit;box-sizing:border-box;outline:none">
